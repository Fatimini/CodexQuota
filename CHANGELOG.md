...
## v1.3.2（2026-09-17，公开测试版）恢复真正平滑的历史曲线

- 修正 v1.3.1 直线连接导致的折线外观：改用单调三次插值，让峰谷和长距离转折连续圆滑，避免插值越过相邻绘图点的数值范围。
- 保留 v1.3.1 的圆角线条、抗锯齿、图表尺寸和取点优化；原始记录与采样规则不变。
- 提取面板实际使用的图表组件，用本机历史直接检查 6 小时、24 小时、7 天以及浅色/深色渲染，不再只看规则生成的示例数据。
- 全量测试 72/72 通过。图表组件渲染已目验；完整菜单栏原生控件交互、休眠与断网恢复仍未完成真机目验。
